...
export default function NewsSwiper({
  ...
  return (
    <div className={` w-full   px-[4%] mb-[4%] mx-auto rounded-[16px] overflow-hidden  ${className}`}>
      <Swiper
      ...
      >
        {items.map((it, i) => (
          <SwiperSlide key={i} aria-roledescription="slide" aria-label={`${i + 1} из ${items.length}`}>
            ...
            <motion.article
              className="grid grid-cols-1 1k:grid-cols-2  "
              variants={slide}
              initial="hidden"
              animate={active === i ? 'show' : 'hidden'}
            >
              {/* левая колонка */}
              <motion.div className="flex flex-col order-2 1k:order-1 justify-center" variants={fadeUp}>
                <motion.div className="flex items-center gap-4 mb-4 text-[#636846]" variants={fadeUp}>
                  {it.tag && (
                    <span className="px-[2%] py-1 rounded-[7px] text-[clamp(0.6875rem,0.5577rem+0.5769vw,1.25rem)] font-ManropeRegular border border-[#636846]">
                      {it.tag}
                    </span>
                  )}
                  {it.time && <span className="rounded-[7px] text-[clamp(0.6875rem,0.5577rem+0.5769vw,1.25rem)] font-ManropeRegular">{it.time}</span>}
                </motion.div>
                ...
                {Array.isArray(it.title) ? (
                  ...
                  <motion.p className="text-[#7b5d44]/90 mb-5  whitespace-pre-line" variants={fadeUp}>
                    {it.title}
                  </motion.p>
                )}

                {Array.isArray(it.excerpt) ? (
                  <div className="text-[#636846] mb-5 font-ManropeRegular text-[clamp(1rem,0.8846rem+0.5128vw,1.5rem)]">
                ...
                {it.cta && (
                  <motion.a
                    href={it.href || '#'}
                    className="inline-flex w-fit items-center justify-center rounded-[5px] bg-[#7B5D44] text-[#F7EFE5] px-[5%] py-3 text-[clamp(0.875rem,0.7885rem+0.3846vw,1.25rem)] font-[Manrope-Regular] duration-700  hover:opacity-90 transition"
              ...
              <motion.div className="flex items-center order-1 1k:order-2 mb-[2%] " variants={fadeUp}>

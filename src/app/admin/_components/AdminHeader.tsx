...
export default function AdminHeader({ title, view, userName, userImage, onAction }: Props) {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {action && (
                <button
                  onClick={onAction}
                  className="flex items-center justify-center gap-1.5 px-3 h-9 bg-[#5C6744] text-white rounded-lg hover:bg-[#4F5938] transition-all hover:scale-105 shadow-sm"
                  aria-label={action.label}
                >
                  <span className="text-base leading-none">{action.icon}</span>
                  <span className="text-xs md:text-sm font-ManropeMedium whitespace-nowrap">{action.label}</span>
                </button>
              )}
      ...
      {/* Menu Modal */}
      <AnimatePresence>
        {menuOpen && <AdminMenu key="admin-menu" setOpen={setMenuOpen} currentView={view} />}
      </AnimatePresence>
    </>
  );
}

...
const MENU_ITEMS = [
  { view: "home" as View, label: "Главная профиля" },
  { view: "booking" as View, label: "Записи на приём" },
  { view: "healthpasses" as View, label: "Пропуски здоровья", badge: 1 },
  { view: "history" as View, label: "История записей" },
  { view: "gifts" as View, label: "Подарочные сертификаты" },
  { view: "settings" as View, label: "Настройки" },
];
...
export default function SideNav({ currentView }: Props) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !searchParams.get("view")) {
      const last = localStorage.getItem("profile_last_view") as View | null;
      if (last && ["home", "booking", "healthpasses", "history", "gifts", "settings"].includes(last)) setView(last);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const Icon = ({ name, active }: { name: View; active: boolean }) => {
    const stroke = active ? COL.olive : COL.olive;
    const common = { stroke, strokeWidth: 1.8, fill: "none", strokeLinecap: "round", strokeLinejoin: "round" } as any;
...
<rect width="32" height="32" fill="white"/>
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className="hidden lg:block w-full lg:w-[280px] xl:w-[320px] 2xl:w-[360px] bg-white border-r flex-shrink-0"
        style={{ borderColor: COL.border }}
      >
        <div className="px-4 xl:px-6 py-6 xl:py-8">
          {/* Лого-заглушка */}
          <div className="mb-16 xl:mb-20 flex items-center gap-2 xl:gap-3">
            <Image
              src="/images/logo.png"
              alt="Логотип"
              width={60}
            ...
            <div>
              <Image onClick={()=>setOpen(!open)}
                            ...
                            width={60}
                            height={60}
                             className="xl:w-[80px] xl:h-[80px] cursor-pointer hover:opacity-80 transition"
                          />
            </div>
          ...
          {/* Навигация */}
          <nav className="flex flex-col gap-4 xl:gap-6 2xl:gap-8" role="navigation" aria-label="Навигация личного кабинета">
            {MENU_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={[
                      "flex items-center justify-between rounded-xl px-3 xl:px-4 py-2.5 xl:py-3 transition-colors",
                      active ? "shadow-none" : "hover:shadow-none hover:bg-[#FAF5EC]",
                    ].join(" ")}
                    style={{
                  ...
                  >
                    <div className="flex items-center gap-2 xl:gap-3">
                      ...
                      <span
                        className="text-[clamp(0.75rem,0.6rem+0.5vw,1rem)] xl:text-[clamp(0.875rem,0.75rem+0.5vw,1.125rem)] font-ManropeMedium text-[#636846]"
                    ...
                    <div className="flex items-center gap-1.5 xl:gap-2">
                      {item.badge ? (
                        <span
                          className="h-4 xl:h-5 min-w-[16px] xl:min-w-[20px] px-1 xl:px-1.5 rounded-full text-[10px] xl:text-[11px] grid place-items-center text-white"
                      ...
                      {active && (
                        <svg className="w-4 h-4 xl:w-[18px] xl:h-[18px]" viewBox="0 0 24 24" fill="none" stroke={COL.olive} strokeWidth="2">
